...
from .form import LoginForm, SignupForm, ChangePasswordForm, PasswordResetRequestForm, PasswordResetForm, ChangeEmailForm
...
def change_password():
    ...
    if form.validate_on_submit():
        if current_user.verify_password(form.old_password.data):
            current_user.password = form.passwrod.data
...
def password_reset(token):
    ...
    if form.validate_on_submit():
        if User.reset_password(token, form.passwrod.data):
            db.session.commit()
            flash('Your password has been updated.')
            return redirect(url_for('auth.login'))
        else:
